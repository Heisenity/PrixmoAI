...
exports.compareAnalyticsPerformanceScores = exports.buildAnalyticsPerformanceScores = void 0;
const SCORE_WEIGHTS = {
...
const TOTAL_WEIGHT = Object.values(SCORE_WEIGHTS).reduce((total, value) => total + value, 0);
const clamp01 = (value) => Math.min(1, Math.max(0, value));
const normalizePercentLikeValue = (value) => {
    if (value === null || !Number.isFinite(value)) {
        return 0;
    }
    return value > 1 ? value / 100 : value;
};
const normalizeSeriesValue = (value, min, max) => {
    if (max === min) {
        return max > 0 ? 1 : 0;
    }
    return clamp01((value - min) / (max - min));
};
const toPublishedTimeValue = (value) => {
    if (!value) {
        return 0;
    }
...
const buildAnalyticsPerformanceScores = (inputs) => {
    const prepared = inputs.map((input) => {
        const normalizer = input.reach > 0
            ? input.reach
            : (input.followersAtPostTime ?? 0) > 0
                ? input.followersAtPostTime
                : input.impressions > 0
                    ? input.impressions
                    : 0;
    ...
    return prepared.map((entry) => {
        const normalizedImpressions = normalizeSeriesValue(entry.impressions, minImpressions, maxImpressions);
        const normalizedReach = normalizeSeriesValue(entry.reach, minReach, maxReach);
        const score = entry.hasSignal
...
exports.buildAnalyticsPerformanceScores = buildAnalyticsPerformanceScores;
const compareAnalyticsPerformanceScores = (left, right) => {
    ...
    const timeDelta = toPublishedTimeValue(right.publishedTime) - toPublishedTimeValue(left.publishedTime);
    if (timeDelta !== 0) {
        return timeDelta;
    }
    return left.id.localeCompare(right.id);
};
exports.compareAnalyticsPerformanceScores = compareAnalyticsPerformanceScores;
