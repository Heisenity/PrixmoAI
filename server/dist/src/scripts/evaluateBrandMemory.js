...
const main = async () => {
    ...
    const [{ data: generationLogs, error: generationLogsError }, { data: feedbackEvents, error: feedbackEventsError }] = await Promise.all([generationLogsQuery, feedbackEventsQuery]);
    if (generationLogsError) {
        throw new Error(generationLogsError.message || 'Failed to load brand memory generation logs');
    }
    if (feedbackEventsError) {
        throw new Error(feedbackEventsError.message || 'Failed to load brand memory feedback events');
    }
    const logs = generationLogs ?? [];
    const feedback = feedbackEvents ?? [];
    const providerCounts = new Map();
    const rerankProviderCounts = new Map();
    ...
    logs.forEach((log) => {
        const provider = typeof log.provider === 'string' && log.provider.trim()
            ? log.provider.trim()
            : 'unknown';
        const rerankProvider = typeof log.rerank_provider === 'string' && log.rerank_provider.trim()
            ? log.rerank_provider.trim()
            : 'none';
        const task = typeof log.task_type === 'string' && log.task_type.trim()
            ? log.task_type.trim()
            : 'unknown';
        providerCounts.set(provider, (providerCounts.get(provider) ?? 0) + 1);
        rerankProviderCounts.set(rerankProvider, (rerankProviderCounts.get(rerankProvider) ?? 0) + 1);
    ...
    const summarizeMap = (map) => [...map.entries()]
    ...
    console.info(JSON.stringify({
        ...
        generationLogs: {
            total: logs.length,
            fallbackRate: logs.length > 0 ? Number((fallbackCount / logs.length).toFixed(4)) : 0,
            averageRetrievedCount: logs.length > 0 ? Number((totalRetrieved / logs.length).toFixed(2)) : 0,
            averageSelectedCount: logs.length > 0 ? Number((totalSelected / logs.length).toFixed(2)) : 0,
...
void main().catch((error) => {
    console.error('[brand-memory-eval] failed', error instanceof Error ? error.message : String(error));
    process.exitCode = 1;
});
