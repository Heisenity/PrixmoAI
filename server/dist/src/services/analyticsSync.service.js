...
exports.startAnalyticsSyncWorker = exports.enqueueAnalyticsSyncJob = exports.syncAnalyticsForUser = void 0;
...
let analyticsSyncUserQueue = null;
let analyticsSyncUserWorker = null;
let analyticsSyncWorkerIdleTimer = null;
...
const readMetadata = (value) => toRecord(value);
const toSyncSocialAccount = (row) => ({
    id: row.id,
    userId: row.user_id,
...
const sumRecordValues = (value) => Object.values(toRecord(value)).reduce((total, entry) => total + toNumber(entry), 0);
const toTopCommentList = (value) => Array.isArray(value)
    ? value
        .map((entry) => {
        ...
        .slice(0, 5)
    : [];
const toBreakdownItems = (value) => Object.entries(toRecord(value))
    .map(([label, rawValue]) => ({
    label,
...
const splitAgeGenderBreakdown = (value) => {
    ...
    for (const [rawKey, rawValue] of Object.entries(toRecord(value))) {
        ...
        const genderLabel = genderCode === 'F'
            ? 'Women'
            : genderCode === 'M'
                ? 'Men'
                : genderCode === 'U'
...
const expandHoursAcrossDays = (hours) => Object.fromEntries(SYNC_DAY_LABELS.flatMap((day) => hours.map((value, hour) => [`${day}-${hour}`, value])));
const normalizeDayLabel = (value) => {
    const normalized = value.trim().slice(0, 3).toLowerCase();
    const match = SYNC_DAY_LABELS.find((label) => label.toLowerCase() === normalized);
    return match ?? null;
};
const toActiveHoursMap = (value) => {
    ...
    if (Array.isArray(firstEntryValue)) {
        return Object.fromEntries(entries.flatMap(([rawDay, rawHours]) => {
            const day = normalizeDayLabel(rawDay);
            if (!day || !Array.isArray(rawHours)) {
                return [];
            }
            return rawHours.map((entry, hour) => [`${day}-${hour}`, toNumber(entry)]);
        }));
    }
    const looksHourly = entries.every(([key]) => /^\d{1,2}$/.test(key));
    if (looksHourly) {
        const hours = Array.from({ length: 24 }, (_, hour) => toNumber(record[String(hour)] ?? record[String(hour).padStart(2, '0')]));
        return expandHoursAcrossDays(hours);
    }
    return Object.fromEntries(entries.map(([key, rawValue]) => [key, toNumber(rawValue)]));
};
...
const fetchGraphJson = async (baseUrl, path, params) => {
    ...
    return (0, retry_1.retryWithBackoff)(async () => {
        ...
        if (!response.ok || payload.error?.message) {
            throw new meta_service_1.MetaGraphApiError(payload.error?.message || 'Meta analytics request failed.', {
        ...
        onRetry: ({ error, attempt, nextDelayMs }) => {
            (0, observability_1.logFailure)('analytics_graph_retry', error, {
...
const metaGraphFetch = (path, params) => fetchGraphJson(GRAPH_BASE_URL, path, params);
const instagramGraphFetch = (path, params) => fetchGraphJson(INSTAGRAM_GRAPH_BASE_URL, path, params);
const extractGraphInsightValue = (insight) => {
    if (!insight) {
        return undefined;
    }
...
const fetchInsightMetrics = async (fetcher, path, accessToken, metrics, extraParams = {}) => {
    const values = {};
    const rawResponses = {};
    const errors = {};
    for (const metric of metrics) {
        try {
            const response = await fetcher(`${path}/insights`, {
...
const getInstagramFetcher = (account) => {
    const metadata = readMetadata(account.metadata);
    const loginType = typeof metadata.instagramLoginType === 'string'
...
const isDirectInstagramLogin = (account) => {
    const metadata = readMetadata(account.metadata);
    return metadata.instagramLoginType === 'instagram_business_login';
};
const getInstagramAnalyticsAccessToken = (account) => {
    const metadata = readMetadata(account.metadata);
    const loginType = typeof metadata.instagramLoginType === 'string'
        ? metadata.instagramLoginType
        : 'facebook_login';
...
const getAlternateInstagramFetcher = (account, primaryFetcher) => isDirectInstagramLogin(account)
    ? null
    : primaryFetcher === instagramGraphFetch
        ? metaGraphFetch
        : instagramGraphFetch;
const getInstagramPostType = (mediaType, mediaProductType, fallbackMediaType) => {
...
const buildFallbackAnalyticsPayload = (account, post, nowIso) => ({
...
const fetchInstagramPostAnalytics = async (account, post) => {
    const metadata = readMetadata(account.metadata);
    ...
    const media = await primaryFetcher(`/${mediaId}`, {
    ...
    const { values: primaryInsights, rawResponses: primaryInsightPayloads, errors: primaryInsightErrors, } = await fetchInsightMetrics(primaryFetcher, `/${mediaId}`, analyticsAccessToken, requestedInsightMetrics);
    const missingInsightMetrics = requestedInsightMetrics.filter((metric) => primaryInsights[metric] === undefined &&
        primaryInsightPayloads[metric] === undefined);
    const { values: fallbackInsights, rawResponses: fallbackInsightPayloads, errors: fallbackInsightErrors, } = missingInsightMetrics.length > 0 && fallbackFetcher
        ? await fetchInsightMetrics(fallbackFetcher, `/${mediaId}`, analyticsAccessToken, missingInsightMetrics)
    ...
    let topComments = [];
    try {
        const comments = await primaryFetcher(`/${mediaId}/comments`, {
            access_token: analyticsAccessToken,
    ...
    const instagramPostType = getInstagramPostType(typeof media.media_type === 'string' ? media.media_type : null, typeof media.media_product_type === 'string' ? media.media_product_type : null, post.mediaType);
    ...
    const hasSavedMetric = insights.saved !== undefined || insightPayloads.saved !== undefined;
    const hasShareMetric = insights.shares !== undefined || insightPayloads.shares !== undefined;
    ...
    const derivedSaves = totalInteractions > 0 && savedMetricUnavailable && !shareMetricUnavailable
    ...
    const derivedShares = totalInteractions > 0 && shareMetricUnavailable && !savedMetricUnavailable
    ...
    const unresolvedInteractionGap = Math.max(0, totalInteractions - likes - comments - resolvedSaves - resolvedShares);
    ...
    if (constants_1.META_OAUTH_DEBUG &&
        ...
        resolvedSaves === 0) {
        console.warn(`[analytics-sync] Instagram saves returned 0 for eligible media ${mediaId}`, {
            postId: post.id,
            platform: account.platform,
            mediaType: typeof media.media_type === 'string' ? media.media_type : null,
            mediaProductType: typeof media.media_product_type === 'string'
    ...
    if (constants_1.META_OAUTH_DEBUG &&
        ...
        console.warn(`[analytics-sync] Instagram saves derived from total interactions for media ${mediaId}`, {
            postId: post.id,
            platform: account.platform,
            likes,
            comments,
    ...
    if (constants_1.META_OAUTH_DEBUG &&
        (instagramPostType === 'image' || instagramPostType === 'reel') &&
        resolvedShares === 0) {
        console.warn(`[analytics-sync] Instagram shares returned 0 for eligible media ${mediaId}`, {
            postId: post.id,
            platform: account.platform,
            mediaType: typeof media.media_type === 'string' ? media.media_type : null,
            mediaProductType: typeof media.media_product_type === 'string'
    ...
    if (constants_1.META_OAUTH_DEBUG && shouldUseDerivedShares) {
        console.warn(`[analytics-sync] Instagram shares derived from total interactions for media ${mediaId}`, {
            postId: post.id,
            platform: account.platform,
            likes,
            comments,
    ...
    if (constants_1.META_OAUTH_DEBUG &&
        ...
        console.warn(`[analytics-sync] Instagram total interactions did not reconcile cleanly for media ${mediaId}`, {
            postId: post.id,
            platform: account.platform,
            likes,
            comments,
    ...
    return {
        ...
        caption: typeof media.caption === 'string' && media.caption.trim()
            ? media.caption
            : post.caption,
        mediaUrl: typeof media.media_url === 'string' && media.media_url.trim()
            ? media.media_url
            : post.mediaUrl,
        thumbnailUrl: typeof media.thumbnail_url === 'string' && media.thumbnail_url.trim()
            ? media.thumbnail_url
        ...
        completionRate: null,
        followersAtPostTime: null,
        engagementRate: reach > 0 ? Number(((engagements / reach) * 100).toFixed(2)) : null,
        publishedTime: toIsoString(media.timestamp) ?? post.publishedAt,
        topComments,
        recordedAt: new Date().toISOString(),
    };
};
const extractFacebookAttachmentMedia = (value) => {
    ...
    return {
        type: typeof firstAttachment.media_type === 'string'
            ? firstAttachment.media_type
            : typeof firstAttachment.type === 'string'
                ? firstAttachment.type
                : null,
        mediaUrl: typeof media.source === 'string'
            ? media.source
            : typeof image.src === 'string'
                ...
                : typeof firstAttachment.unshimmed_url === 'string'
                    ...
                    : typeof firstAttachment.url === 'string'
                        ? firstAttachment.url
                        : null,
        thumbnailUrl: typeof image.src === 'string'
            ? image.src
...
const fetchFacebookPostAnalytics = async (account, post) => {
    if (!account.accessToken || !post.externalPostId) {
        return buildFallbackAnalyticsPayload(account, post, new Date().toISOString());
    }
    if (post.mediaType === 'video') {
        const video = await metaGraphFetch(`/${post.externalPostId}`, {
            access_token: account.accessToken,
            fields: 'id,description,created_time,picture,reactions.summary(total_count).limit(0),comments.summary(total_count).limit(0)',
        });
        const { values: insights } = await fetchInsightMetrics(metaGraphFetch, `/${post.externalPostId}`, account.accessToken, [
        ...
        let topComments = [];
        try {
            const comments = await metaGraphFetch(`/${post.externalPostId}/comments`, {
                access_token: account.accessToken,
        ...
        const reactions = toNumber(toRecord(toRecord(video.reactions).summary).total_count);
        const comments = toNumber(toRecord(toRecord(video.comments).summary).total_count);
        const reach = toNumber(insights.total_video_impressions_unique);
        let shares = 0;
        try {
            const videoShareSnapshot = await metaGraphFetch(`/${post.externalPostId}`, {
                access_token: account.accessToken,
                fields: 'shares',
            });
        ...
        return {
            ...
            postExternalId: post.externalPostId,
            postType: 'video',
            caption: typeof video.description === 'string' && video.description.trim()
                ? video.description
                : post.caption,
            mediaUrl: post.mediaUrl,
            thumbnailUrl: typeof video.picture === 'string' && video.picture.trim()
                ? video.picture
                : post.mediaUrl,
            reach,
            ...
            completionRate: null,
            followersAtPostTime: null,
            engagementRate: reach > 0 ? Number((((comments + shares + reactions) / reach) * 100).toFixed(2)) : null,
            publishedTime: toIsoString(video.created_time) ?? post.publishedAt,
            topComments,
            recordedAt: new Date().toISOString(),
        };
    }
    const feedPost = await metaGraphFetch(`/${post.externalPostId}`, {
        access_token: account.accessToken,
        fields: 'id,message,created_time,full_picture,shares,reactions.summary(total_count).limit(0),comments.summary(total_count).limit(0),attachments{media,target,url,unshimmed_url,media_type,type}',
    });
    const { values: insights } = await fetchInsightMetrics(metaGraphFetch, `/${post.externalPostId}`, account.accessToken, [
        'post_impressions',
    ...
    const likes = toNumber(actionBreakdown.like) ||
        toNumber(actionBreakdown.likes) ||
        0;
    const comments = toNumber(toRecord(toRecord(feedPost.comments).summary).total_count);
    const reactions = sumRecordValues(reactionsByType) ||
        toNumber(toRecord(toRecord(feedPost.reactions).summary).total_count);
    const shares = toNumber(toRecord(feedPost.shares).count) ||
    ...
    return {
        ...
        platform: 'facebook',
        postExternalId: post.externalPostId,
        postType: attachmentMedia.type?.toLowerCase().includes('video')
            ? 'video'
            : 'image',
        caption: typeof feedPost.message === 'string' && feedPost.message.trim()
            ? feedPost.message
            : post.caption,
        mediaUrl: attachmentMedia.mediaUrl ||
            (typeof feedPost.full_picture === 'string' ? feedPost.full_picture : post.mediaUrl),
        thumbnailUrl: attachmentMedia.thumbnailUrl ||
            (typeof feedPost.full_picture === 'string' ? feedPost.full_picture : post.mediaUrl),
        reach,
...
const fetchMetaPostAnalytics = async (account, post) => {
...
const fetchInstagramAudienceSnapshot = async (account) => {
    const metadata = readMetadata(account.metadata);
    const instagramAccountId = typeof metadata.metaInstagramAccountId === 'string'
        ? metadata.metaInstagramAccountId
        : null;
    ...
    const profile = await fetcher(`/${instagramAccountId}`, {
        access_token: analyticsAccessToken,
        fields: 'id,followers_count,media_count',
    });
    const { values: dayInsights } = await fetchInsightMetrics(fetcher, `/${instagramAccountId}`, analyticsAccessToken, ['follower_count', 'impressions', 'reach', 'profile_views'], { period: 'day' });
    const { values: lifetimeInsights } = await fetchInsightMetrics(fetcher, `/${instagramAccountId}`, analyticsAccessToken, ['audience_gender_age', 'audience_city', 'online_followers'], { period: 'lifetime' });
    const ageGender = splitAgeGenderBreakdown(dayInsights.audience_gender_age ?? lifetimeInsights.audience_gender_age);
    return {
        socialAccountId: account.id,
        platform: 'instagram',
        followers: toNumber(dayInsights.follower_count) || toNumber(profile.followers_count),
...
const fetchFacebookAudienceSnapshot = async (account) => {
    const metadata = readMetadata(account.metadata);
    const pageId = typeof metadata.metaPageId === 'string' ? metadata.metaPageId : null;
    if (!pageId || !account.accessToken) {
        return null;
    }
    const page = await metaGraphFetch(`/${pageId}`, {
        access_token: account.accessToken,
        fields: 'id,followers_count,fan_count',
    });
    const { values: pageInsights } = await fetchInsightMetrics(metaGraphFetch, `/${pageId}`, account.accessToken, [
        'page_impressions',
    ...
    return {
        socialAccountId: account.id,
        platform: 'facebook',
        followers: toNumber(page.followers_count) || toNumber(pageInsights.page_fans),
...
const fetchMetaAudienceSnapshot = async (account) => {
...
const syncAnalyticsForUser = async (client, userId, options = {}) => {
    ...
    const socialAccounts = (accountsResult.data ?? [])
        .map((row) => toSyncSocialAccount(row))
        .filter((account) => options.socialAccountIds?.length ? options.socialAccountIds.includes(account.id) : true);
    ...
    const handleAccountSyncFailure = async (account, error, context, entityId) => {
        if (account.platform !== 'instagram' && account.platform !== 'facebook') {
            return error instanceof Error ? error.message : 'Analytics sync failed.';
        }
        const normalized = (0, meta_service_1.normalizeMetaFailureForUser)(error, account.platform);
        (0, observability_1.logFailure)('analytics_account_sync_failed', error, {
            userId,
            provider: 'meta',
            platform: account.platform,
            socialAccountId: account.id,
        ...
        (0, observability_1.recordFailureSpikeSignal)('analytics_account_sync_failed', {
            userId,
            provider: 'meta',
            platform: account.platform,
            failureKind: normalized.kind,
        ...
        if (normalized.accountStatus) {
            ...
            const { error: statusError } = await client
                ...
                .eq('user_id', userId);
            if (statusError) {
                (0, observability_1.logFailure)('analytics_account_status_update_failed', statusError, {
                    userId,
                    provider: 'meta',
                    platform: account.platform,
                    socialAccountId: account.id,
    ...
    for (const post of posts) {
        ...
        try {
            ...
            await (0, analytics_1.saveAnalyticsData)(client, userId, analyticsPayload);
            summary.postsSynced += 1;
        }
        catch (error) {
            const message = await handleAccountSyncFailure(account, error, 'post', post.id);
            summary.errors.push(`Post ${post.id}: ${message}`);
        }
    }
    for (const account of socialAccounts) {
        ...
        try {
            ...
            const savedSnapshot = await (0, analytics_1.saveAnalyticsAudienceSnapshot)(client, userId, snapshot);
            if (savedSnapshot) {
                summary.audienceSnapshotsSynced += 1;
            }
        }
        catch (error) {
            const message = await handleAccountSyncFailure(account, error, 'audience', account.id);
            summary.errors.push(`Audience ${account.id}: ${message}`);
        }
    }
...
const getAnalyticsSyncUserQueue = () => {
    if (!analyticsSyncUserQueue) {
        analyticsSyncUserQueue = new bullmq_1.Queue(queueNames_1.QUEUE_NAMES.analyticsSyncUser, (0, redis_1.getBullMqConfig)('prixmoai:queue:analytics-user'));
    }
...
const scheduleAnalyticsSyncWorkerIdleShutdown = () => {
    if (constants_1.ANALYTICS_WORKER_IDLE_SHUTDOWN_MS <= 0 || !analyticsSyncUserWorker) {
        return;
    }
    ...
    analyticsSyncWorkerIdleTimer = setTimeout(() => {
        ...
        void workerToClose.close().catch((error) => {
            console.error(`[analytics-sync] ${error instanceof Error
                ? error.message
...
const enqueueAnalyticsSyncJob = async (data) => {
    if (!redis_1.isRedisConfigured) {
        throw new Error('Redis is not configured. Set REDIS_URL (or UPSTASH_REDIS_URL) before starting analytics sync jobs.');
    }
    await getAnalyticsSyncUserQueue().add('sync-user', data, {
        jobId: [
            'analytics',
            data.userId,
    ...
    (0, observability_1.logOperationalEvent)('analytics_sync_job_enqueued', {
        userId: data.userId,
        queue: queueNames_1.QUEUE_NAMES.analyticsSyncUser,
        postIds: data.postIds ?? null,
        socialAccountIds: data.socialAccountIds ?? null,
    });
    (0, exports.startAnalyticsSyncWorker)();
};
exports.enqueueAnalyticsSyncJob = enqueueAnalyticsSyncJob;
const startAnalyticsSyncWorker = () => {
    ...
    analyticsSyncUserWorker = new bullmq_1.Worker(queueNames_1.QUEUE_NAMES.analyticsSyncUser, async (job) => {
        const client = (0, supabase_1.requireSupabaseAdmin)();
        try {
            return await (0, exports.syncAnalyticsForUser)(client, job.data.userId, {
            ...
            (0, observability_1.logFailure)('analytics_sync_job_failed', error, {
                jobId: job.id,
                userId: job.data.userId,
                queue: queueNames_1.QUEUE_NAMES.analyticsSyncUser,
                provider: 'meta',
            });
            (0, observability_1.recordFailureSpikeSignal)('analytics_sync_job_failed', {
                queue: queueNames_1.QUEUE_NAMES.analyticsSyncUser,
                provider: 'meta',
            });
            throw error;
        ...
        ...(0, redis_1.getBullMqConfig)('prixmoai:worker:analytics-user'),
        ...(0, workerOptions_1.getLowCommandWorkerOptions)(),
        concurrency: constants_1.ANALYTICS_SYNC_JOB_CONCURRENCY,
    });
    ...
    analyticsSyncUserWorker.on('failed', (job, error) => {
        (0, observability_1.logFailure)('analytics_sync_worker_failed', error, {
            jobId: job?.id ?? null,
...
exports.startAnalyticsSyncWorker = startAnalyticsSyncWorker;
