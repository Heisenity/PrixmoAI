...
const HTML_ATTRIBUTE_PATTERN = /([a-zA-Z:-]+)\s*=\s*("([^"]*)"|'([^']*)'|([^\s"'=<>`]+))/g;
...
const inferMediaTypeFromContentType = (contentType) => ALLOWED_SOURCE_VIDEO_TYPES.has(contentType) ? 'video' : 'image';
const getMaxBytesForContentType = (contentType) => inferMediaTypeFromContentType(contentType) === 'video'
...
const extractEmbeddedMediaUrlFromPageUrl = (pageUrl) => {
    try {
...
const extractPreviewMediaUrlFromHtml = (html, pageUrl) => {
...
const assertAllowedMediaType = (contentType) => {
...
const readResponseContentType = (response) => response.headers.get('content-type')?.split(';')[0]?.trim().toLowerCase() ?? null;
const parseMediaDataUrl = (dataUrl) => {
...
const uploadSourceImageBuffer = async (userId, input) => {
...
const inferRemoteFileName = (sourceUrl, contentType) => {
    try {
...
const resolveExternalSourceImage = async (sourceUrl) => {
    const resolveFromUrl = async (url, depth, wasExtracted) => {
        if (depth > 2) {
...
const importExternalSourceImage = async (userId, sourceUrl) => {
    ...
    const contentTypeHeader = readResponseContentType(response);
    const contentType = assertAllowedMediaType(contentTypeHeader || resolvedMedia.contentType);
...
exports.importExternalSourceImage = importExternalSourceImage;
