...
exports.collectRealtimeTrendIntelligence = void 0;
const crypto_1 = require("crypto");
const constants_1 = require("../config/constants");
const requestCancellation_1 = require("../lib/requestCancellation");
const trendResearchCache = new Map();
...
const PLATFORM_HALF_LIFE_HOURS = {
    instagram: 72,
    facebook: 84,
    linkedin: 96,
    x: 20,
...
const dedupeStrings = (values) => unique(values.map((value) => normalizeText(value)).filter(Boolean));
const extractHashtags = (value) => unique((value.match(/#[\p{L}\p{N}_][\p{L}\p{N}_-]*/gu) ?? []).map((tag) => tag.toLowerCase()));
const safeNumber = (value) => typeof value === 'number'
    ? value
    : typeof value === 'string'
        ? Number.parseFloat(value.replace(/,/g, ''))
        : NaN;
const clamp01 = (value) => Math.min(1, Math.max(0, value));
const softScore = (value, pivot) => value > 0 ? value / (value + Math.max(1, pivot)) : 0;
const hoursBetween = (date, now = Date.now()) => Math.max(0, (now - date.getTime()) / (1000 * 60 * 60));
...
const pickStringArray = (record, keys) => {
    for (const key of keys) {
        const value = record[key];
...
const buildApifyRowPreview = (payload) => {
...
const buildTokenWeights = (brandProfile, productInput, brandMemories) => {
    const weightedTokens = [];
    const pushWeightedTokens = (text, weight) => {
        if (!text) {
            return;
        }
...
const buildSearchQueries = (purpose, brandProfile, productInput, brandMemories) => {
    ...
    const industry = brandProfile?.primaryIndustry?.trim() ||
        brandProfile?.industry?.trim() ||
        '';
    const purposeLabel = purpose === 'image-generation'
        ? 'visual campaign'
    ...
    return queries.slice(0, constants_1.TAVILY_SEARCH_QUERY_LIMIT);
};
const buildCacheKey = (purpose, userId, productInput, queries) => JSON.stringify({
    purpose,
    userId,
...
const createTimeoutSignal = (timeoutMs, baseSignal) => {
...
const fetchJson = async (url, init, timeoutMs, signal) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Request failed (${response.status}): ${isRecord(payload) && isRecord(payload.error)
...
const computeTopicMatchScore = (text, tokenWeights) => {
...
const computeFreshnessScore = (platform, publishedAt) => {
    if (!publishedAt) {
        return 0.35;
    }
...
const computeShareRatioScore = (shares, views, likes) => {
    if (shares <= 0) {
        return 0;
    }
...
const computeCommentIntensityScore = (comments, likes, views) => {
    if (comments <= 0) {
        return 0;
    }
...
const computeCreatorStreakScore = (followers, likes, comments, shares, views) => {
...
const computeSentimentBoostScore = (text) => {
...
const computeQualityScore = (source, text, url) => {
...
const computeViralScore = (source, topicMatchScore, freshnessScore, shareRatioScore, commentIntensityScore, creatorStreakScore, sentimentBoostScore, qualityScore) => clamp01(topicMatchScore * 0.27 +
    freshnessScore * 0.19 +
...
const buildReasons = (scores) => scores
...
const normalizeCandidate = ({ source, platform, title, text, url, authorName, hashtags, publishedAt, metrics, tokenWeights, }) => {
    const normalizedText = normalizeText(text);
    const qualityScore = computeQualityScore(source, [title, normalizedText].filter(Boolean).join(' '), url);
    if (qualityScore <= 0.12) {
        return null;
    }
    const topicMatchScore = computeTopicMatchScore([title, normalizedText, hashtags.join(' ')].filter(Boolean).join(' '), tokenWeights);
    const freshnessScore = computeFreshnessScore(platform, publishedAt);
    const shareRatioScore = computeShareRatioScore(metrics.shares, metrics.views, metrics.likes);
    const commentIntensityScore = computeCommentIntensityScore(metrics.comments, metrics.likes, metrics.views);
    const creatorStreakScore = computeCreatorStreakScore(metrics.followers, metrics.likes, metrics.comments, metrics.shares, metrics.views);
    const sentimentBoostScore = computeSentimentBoostScore([title, normalizedText].filter(Boolean).join(' '));
    const viralScore = computeViralScore(source, topicMatchScore, freshnessScore, shareRatioScore, commentIntensityScore, creatorStreakScore, sentimentBoostScore, qualityScore);
    const ageHours = publishedAt && !Number.isNaN(new Date(publishedAt).getTime())
...
const parseTavilyCandidates = (payload, tokenWeights) => {
    ...
    return payload.results
        ...
        return normalizeCandidate({
            ...
            publishedAt: parsedAge && !Number.isNaN(parsedAge.getTime())
...
const readApifyConfig = (platform) => {
...
const readApifyPlatformSecret = (platform, suffix) => {
    const normalized = normalizePlatform(platform);
    if (!normalized) {
        return null;
    }
...
const parseApifySecretValue = (value) => {
    if (!value) {
        return null;
    }
    try {
...
const buildApifyEndpoint = (config, token) => config.kind === 'task'
    ? `${constants_1.APIFY_API_BASE_URL}/v2/actor-tasks/${config.id}/run-sync-get-dataset-items?token=${encodeURIComponent(token)}&format=json&clean=true`
    : `${constants_1.APIFY_API_BASE_URL}/v2/acts/${config.id}/run-sync-get-dataset-items?token=${encodeURIComponent(token)}&format=json&clean=true`;
const buildApifyInput = (purpose, platform, queries, brandProfile, productInput) => {
    ...
    const baseInput = {
        ...
        resultsLimit: constants_1.APIFY_RESULT_LIMIT,
        maxItems: constants_1.APIFY_RESULT_LIMIT,
        maxResults: constants_1.APIFY_RESULT_LIMIT,
        maxPosts: constants_1.APIFY_RESULT_LIMIT,
        maxRequestRetries: 1,
    };
    ...
    ].filter((value) => typeof value === 'string' && value.trim().length > 0)).slice(0, 6);
    const startUrls = dedupeStrings([
        ...usernameCandidates.map((handle) => `https://www.facebook.com/${handle}`),
        ...searchQueries.map((query) => `https://www.facebook.com/search/posts/?q=${encodeURIComponent(query)}`),
    ]).map((url) => ({ url }));
    return {
        ...baseInput,
        startUrls,
...
const parseApifyCandidates = (payload, platform, tokenWeights) => {
...
const buildInsights = (candidates) => candidates.slice(0, constants_1.TREND_RESEARCH_MAX_INSIGHTS).map((candidate) => ({
    headline: candidate.title ??
        clampText(candidate.text.split(/(?<=[.!?])\s+/)[0] ?? candidate.text, 90),
    explanation: candidate.reasons.length > 0
...
const buildTopHashtags = (candidates) => {
    const weighted = new Map();
    for (const candidate of candidates) {
        for (const hashtag of candidate.hashtags) {
            weighted.set(hashtag, (weighted.get(hashtag) ?? 0) + candidate.viralScore + candidate.topicMatchScore);
        }
    }
...
const buildSummary = (purpose, candidates, topHashtags) => {
    if (!candidates.length) {
        return `No live trend signals were strong enough to use for ${purpose}.`;
    }
    const platforms = unique(candidates.map((candidate) => candidate.platform).filter(Boolean));
    const strongestReasons = unique(candidates.flatMap((candidate) => candidate.reasons.map((reason) => reason.label))).slice(0, 4);
    return [
...
const fetchTavilyTrendSignals = async (queries, tokenWeights, signal) => {
    const apiKey = process.env.TAVILY_API_KEY?.trim();
    if (!apiKey) {
        return [];
    }
    const responses = await Promise.all(queries.map(async (query) => {
        const payload = await fetchJson(`${constants_1.TAVILY_SEARCH_API_BASE_URL}/search`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                max_results: constants_1.TAVILY_SEARCH_RESULT_LIMIT,
        ...
        }, constants_1.TAVILY_SEARCH_TIMEOUT_MS, signal);
        return parseTavilyCandidates(payload, tokenWeights);
    }));
    return responses.flat();
};
const fetchApifyTrendSignals = async (purpose, platform, queries, brandProfile, productInput, tokenWeights, signal) => {
    ...
    const payload = await fetchJson(buildApifyEndpoint(config, token), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify(buildApifyInput(purpose, config.platform, queries, brandProfile, productInput)),
    }, constants_1.APIFY_TIMEOUT_MS, signal);
...
const collectRealtimeTrendIntelligence = async ({ purpose, userId, brandProfile, productInput, brandMemories, signal, }) => {
    (0, requestCancellation_1.throwIfRequestCancelled)(signal);
    const queries = buildSearchQueries(purpose, brandProfile, productInput, brandMemories);
    ...
    const tokenWeights = buildTokenWeights(brandProfile, productInput, brandMemories);
    ...
    try {
        const [nextWebCandidates, apifyResult] = await Promise.all([
            fetchTavilyTrendSignals(queries, tokenWeights, signal).catch((error) => {
                ...
                tavilyDiagnostics.reasonIfSkipped = `request_failed:${error instanceof Error ? error.message : String(error)}`;
            ...
            apifyDisabledForPlatform
                ...
                : fetchApifyTrendSignals(purpose, selectedPlatform, queries, brandProfile, productInput, tokenWeights, signal).catch((error) => {
                    apifyDiagnostics.used = false;
                    apifyDiagnostics.reasonIfSkipped = `request_failed:${error instanceof Error ? error.message : String(error)}`;
    ...
    const topCandidates = uniqueCandidates.slice(0, constants_1.TREND_RESEARCH_MAX_CANDIDATES);
    ...
    trendResearchCache.set(cacheKey, {
        expiresAt: Date.now() + constants_1.TREND_RESEARCH_CACHE_TTL_MS,
        value: intelligence,
    });
    console.info('[trend-intelligence] research completed', {
        userId,
...
exports.collectRealtimeTrendIntelligence = collectRealtimeTrendIntelligence;
