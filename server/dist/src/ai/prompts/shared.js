...
exports.formatTrendIntelligence = exports.formatProductContext = exports.formatBrandContext = exports.formatRetrievedBrandMemories = void 0;
const formatList = (items) => items && items.length > 0 ? items.join(', ') : 'none provided';
const formatPlatformWritingPolicy = (platform) => {
...
const clampMemoryText = (value, maxChars = 260) => {
...
const formatRetrievedBrandMemories = (brandMemories) => {
...
const formatBrandContext = (brandProfile, brandMemories) => {
    const semanticMemorySection = (0, exports.formatRetrievedBrandMemories)(brandMemories);
    if (!brandProfile) {
        const lines = [
...
const formatTrendIntelligence = (trendIntelligence) => {
    ...
    return [
        ...
        `- Fresh hashtags emerging now: ${trendIntelligence.topHashtags.length > 0
            ? trendIntelligence.topHashtags.join(', ')
            : 'none strong enough'}`,
        ...trendIntelligence.insights.slice(0, 4).map((insight, index) => `- Trend insight ${index + 1}: ${insight.headline} | ${insight.explanation} | source=${insight.source}${insight.platform ? `/${insight.platform}` : ''} | score=${insight.viralScore.toFixed(2)}`),
        ...trendIntelligence.topCandidates.slice(0, 4).map((candidate, index) => `- High-signal example ${index + 1}: ${clampMemoryText([candidate.title, candidate.text].filter(Boolean).join(' — '), 220)} | reasons=${candidate.reasons.map((reason) => reason.label).join(', ') || 'general relevance'}`),
...
exports.formatTrendIntelligence = formatTrendIntelligence;
