...
const LANGUAGE_TRANSCRIPTION_GUIDES = {
    bn: {
        ...
        cleanupPrompt: 'বাংলা/Banglish ASR টেক্সট। স্পষ্ট বানান, spacing, খুব দরকারি ন্যূনতম punctuation, phonetic ASR ভুল, আর clear overlap repetition ঠিক করো। Bengali grammar/common words বাংলা করো: জেটা -> যেটা, সাম্নে -> সামনে, ছবী -> ছবি। কিন্তু spoken English creative words English রাখো বা ফিরিয়ে দাও: খ্রাইম/ক্রাইম -> crime, কল্ট/কাল্ট -> cult, থ্রিলার/থ্রিলের -> thriller, কন্টেন্ট -> content, পোস্টার/পোস্টের -> poster, ফিফটিজ -> fifties, ইন্সপায়ার্ড/ইন্স্পারের -> inspired, সিনেমা/সিনমা -> cinema when used as an English creative word। একই অর্থ ও order রাখো। unsure হলে original শব্দ রাখো।',
...
const getCleanupScriptInstruction = (language, scriptName) => {
    if (language === 'en') {
        return null;
    }
    if (language === 'bn') {
...
const replaceStandaloneToken = (value, tokenPattern, replacement) => value.replace(new RegExp(`(^|[^\\p{L}\\p{N}])(?:${tokenPattern})(?=$|[^\\p{L}\\p{N}])`, 'giu'), (_match, prefix) => `${prefix}${replacement}`);
const removeBengaliSpeechJunkTail = (value) => normalizeTranscriptText(value)
    .replace(/(?:,?\s*)?(?:fortunately|unfortunately|com|\.com|come)(?:[,.\s]*)$/iu, '')
    .trim();
...
const normalizeBengaliSpeechArtifacts = (value) => {
    let normalized = normalizeTranscriptText(value)
        ...
        .replace(/\b(crime|cult|thriller|content|poster|fifties|cinema|vintage|style|hero|inspired)\b/giu, (term) => term.toLowerCase());
    ...
    normalized = normalized
        ...
        .replace(/(^|[^\p{L}\p{N}])ব্যানার\s+বান[িী]\s+দাও/giu, (_match, prefix) => `${prefix}ব্যানার দাও`)
...
const applyLanguageCorrections = (value, language) => {
    ...
    const speechArtifactNormalized = language === 'bn' ? normalizeBengaliSpeechArtifacts(baseline) : baseline;
