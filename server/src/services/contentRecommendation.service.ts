...
import { createBrandMemoryGenerationLog } from '../db/queries/brandMemorySignals';
import type {
  BrandMemoryMatch,
  BrandProfile,
  CaptionVariant,
...
import { getRelevantMemoriesForSchedulingRecommendation } from './brandMemory.service';

const recommendationSchema = z.object({
  selectedVariantIndex: z.number().int().min(1).max(12),
  recommendedCaption: z.string().trim().min(16).max(2200),
  reasoning: z.string().trim().min(8).max(600),
  note: z.string().trim().min(8).max(220),
  supportingMemoryIds: z.array(z.string().uuid()).max(5).default([]),
});

const clampText = (value: string, maxChars = 240) => {
...
const buildCaptionBody = (caption: CaptionVariant) =>
  [caption.hook, caption.mainCopy, caption.cta].filter(Boolean).join('\n\n').trim();

const buildPlatformMemoryContext = (
  platform: string | null | undefined,
  retrievedMemories: BrandMemoryMatch[]
) => {
  ...
  return retrievedMemories
    .filter((memory) => {
      const memoryPlatform =
        typeof memory.metadata.platform === 'string'
      ...
      return (
        memory.memoryType === 'platform-performance-insight' &&
...
const buildRecommendationPrompt = (input: {
  ...
  (() => {
    const platformMemoryContext = buildPlatformMemoryContext(
    ...
    `Platform-specific memory cues: ${
      platformMemoryContext.length > 0
...
const buildFallbackRecommendation = (input: {
  ...
  const scoredCandidates = input.captions.map((caption, index) => {
    ...
    const hookBoost = lexicalOverlapScore(
      `${input.content.goal ?? ''} ${input.content.platform ?? ''}`,
      caption.hook
    );
    const ctaBoost = lexicalOverlapScore(
      `${input.content.goal ?? ''} ${input.content.keywords?.join(' ') ?? ''}`,
      caption.cta
    );
    const platformMemoryBoost = lexicalOverlapScore(
      platformMemoryContext,
      captionText
    );
  ...
  return {
    ...
    reasoning:
      'This variation best matched the current product brief, goal, and the strongest reusable brand signals available.',
    note:
      'PrixmoAI picked this caption because it lines up best with your current goal, brand voice, and past performance signals.',
    strategy: 'fallback',
    provider: 'fallback',
...
export const recommendCaptionForScheduling = async (
...
): Promise<ScheduleCaptionRecommendation> => {
  ...
  const retrieval = await getRelevantMemoriesForSchedulingRecommendation(
    client,
    input.userId,
    input.brandProfile,
    {
  ...
  try {
    const result = await generateStructuredDataWithGroqFallback(
      prompt,
      recommendationSchema,
      'memory-rerank'
    );
    const selectedVariantIndex = Math.min(
      Math.max(result.data.selectedVariantIndex, 1),
      input.content.captions.length
    );
    ...
    const observabilityLog = await createBrandMemoryGenerationLog(client, {
      ...
      selectedMemories: [
        ...retrieval.telemetry.selectedMemories,
        {
          ...
          recommendedCaption: clampText(
            fallbackRecommendation.recommendedCaption,
            400
          ),
        },
      ],
