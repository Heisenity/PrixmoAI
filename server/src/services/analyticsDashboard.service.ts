  ...
  for (let dayIndex = 0; dayIndex < HEATMAP_DAY_LABELS.length; dayIndex += 1) {
    for (let hour = 0; hour < 24; hour += 1) {
      ...
      heatmap.push({
        ...
        intensity:
          averageEngagementRate !== null && maxRate > 0
...
export const getAnalyticsDashboard = async (
  ...
  const scheduledPosts = (scheduledPostsData ?? []) as ScheduledPostAnalyticsRow[];
  ...
  const posts: AnalyticsPostInsight[] = currentSnapshots
    .map((row) => {
      ...
      const normalizedReach =
        maxReach === minReach ? 1 : (row.reach - minReach) / (maxReach - minReach);
      const performanceScore =
        (row.engagementRateRatio ?? 0) * 0.5 +
        normalizedImpressions * 0.3 +
        normalizedReach * 0.2;
