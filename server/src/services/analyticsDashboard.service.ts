...
import { ANALYTICS_LEARNING_MIN_POSTS } from '../config/constants';
import {
  getAnalyticsAudienceSnapshotsByUserId,
  getAnalyticsByUserId,
} from '../db/queries/analytics';
import { listAnalyticsLearningProfilesByUser } from '../db/queries/analyticsLearning';
import type {
  AnalyticsAudienceBreakdownItem,
  AnalyticsAudienceSnapshot,
  AnalyticsBestTimeInsight,
  AnalyticsDashboard,
  AnalyticsLearningDashboard,
  AnalyticsLearningPattern,
  AnalyticsLearningProfile,
  AnalyticsDashboardDateRange,
...
import {
  ...
  compareAnalyticsPerformanceScores,
} from '../lib/analyticsPerformance';
...
const MINIMUM_BEST_TIME_POSTS = ANALYTICS_LEARNING_MIN_POSTS;
...
type AnalyticsDashboardOptions = {
  preset?: keyof typeof PRESET_DAY_MAP | 'custom';
  start?: string;
  end?: string;
  platformScope?: AnalyticsPlatformScope;
};

type ScheduledPostAnalyticsRow = {
  id: string;
...
const resolveScheduledPostPublishedAt = (
  row: Pick<ScheduledPostAnalyticsRow, 'status' | 'published_at' | 'updated_at'>
) => row.published_at ?? (row.status === 'published' ? row.updated_at : null);

type SocialAccountAnalyticsRow = {
  id: string;
  platform: string;
...
type DateRangeWindow = {
  preset: AnalyticsDashboardDateRange['preset'];
  start: string;
  end: string;
...
type EnrichedAnalyticsRow = AnalyticsData & {
  platformKey: Exclude<AnalyticsPlatformScope, 'all'> | null;
  platformLabel: string;
  socialAccountId: string | null;
  socialAccountName: string | null;
  postKey: string;
  engagements: number;
  engagementRateRatio: number | null;
};
...
const toPlatformLabel = (value: string | null | undefined) => {
...
const formatLearningPatternLabel = (dimension: string, value: string) => {
...
const inferContentThemes = (caption: string | null, postType: string | null) => {
          ...
          typeof entry.label === 'string'
            ...
            : typeof entry.name === 'string'
              ...
              : typeof entry.key === 'string'
                ...
                : null;
        const numericValue =
          toNullableNumber(entry.value ?? entry.count ?? entry.total ?? entry.percentage) ?? 0;
...
const mergeAudienceBreakdownItems = (
  collections: AnalyticsAudienceBreakdownItem[][]
): AnalyticsAudienceBreakdownItem[] => {
...
const buildFollowerGrowthSeriesFromAudienceSnapshots = (
  ...
  return [...dailyBuckets.entries()]
    .map(([date, snapshotsForDay]) => {
      const latestPerAccount = latestByKey(
        snapshotsForDay,
        (snapshot) => snapshot.socialAccountId,
        (snapshot) => snapshot.recordedAt
      );

      return {
        date,
        label: toDateLabel(date),
        value: latestPerAccount.reduce(
          (total, snapshot) => total + snapshot.followers,
          0
        ),
      };
    })
...
const aggregateLatestAudienceMetric = (
  snapshots: AnalyticsAudienceSnapshot[],
  selector: (snapshot: AnalyticsAudienceSnapshot) => number
) =>
  latestByKey(
    snapshots,
    (snapshot) => snapshot.socialAccountId,
    (snapshot) => snapshot.recordedAt
  ).reduce((total, snapshot) => total + selector(snapshot), 0);

const buildActiveHoursHeatmapFromSnapshots = (
  snapshots: AnalyticsAudienceSnapshot[],
  fallback: AnalyticsHeatmapCell[]
) => {
  const latestSnapshots = latestByKey(
    snapshots,
    (snapshot) => snapshot.socialAccountId,
    (snapshot) => snapshot.recordedAt
  );
...
const collectAudienceBreakdown = (
  accounts: SocialAccountAnalyticsRow[],
  candidateKeys: string[]
) =>
  mergeAudienceBreakdownItems(
    accounts.map((account) => readMetadataBreakdown(account.metadata, candidateKeys))
  );
...
const getPostKey = (row: Pick<AnalyticsData, 'scheduledPostId' | 'postExternalId' | 'id'>) =>
  row.scheduledPostId || row.postExternalId || row.id;

const getEngagements = (
  row: Pick<AnalyticsData, 'likes' | 'comments' | 'saves' | 'shares' | 'reactions'>
) => row.likes + row.comments + row.saves + row.shares + row.reactions;

const toRateRatio = (rate: number | null, reach: number, engagements: number) => {
...
const computeChangePercent = (current: number | null, previous: number | null) => {
...
const toDirection = (changePercent: number | null): AnalyticsMetricValue['direction'] => {
...
const buildTrendBuckets = (
  ...
  for (let cursor = new Date(range.start); cursor < new Date(range.end); cursor = addIstDays(cursor, 1)) {
    ...
    const platformBreakdown: AnalyticsTrendPoint['platformBreakdown'] = {};
...
const extractKeywords = (caption: string | null) => {
  if (!caption) {
    return [];
  }
...
const buildSparkline = (
  ...
  selector: (point: AnalyticsTrendPoint) => number
): AnalyticsMetricPoint[] =>
  points.map((point) => ({
    date: point.date,
    label: point.label,
...
const buildHeatmap = (posts: AnalyticsPostInsight[]): AnalyticsBestTimeInsight => {
  ...
  for (const post of posts) {
    ...
    const normalizer =
      post.reach > 0 ? post.reach : post.impressions > 0 ? post.impressions : 0;
    ...
    const weightedActions =
      appreciationActions + post.comments * 2 + post.saves * 3 + post.shares * 4;
  ...
  const scoreMad = median(
    scoredTimingPosts.map((entry) => Math.abs(entry.rawScore - scoreMedian))
  );
  const robustStdEstimate = scoreMad * 1.4826;
  const outlierCap =
    scoreMedian > 0 && scoreMad === 0
  ...
  for (const scoredPost of scoredTimingPosts) {
    const cappedScore =
      outlierCap > 0 ? Math.min(scoredPost.rawScore, outlierCap) : scoredPost.rawScore;
  ...
  const hasRepeatableWinner = Boolean(
    headline && headline.qualifiedPosts >= MINIMUM_BEST_TIME_SLOT_SAMPLES
  );
  const signalStatus: AnalyticsBestTimeInsight['signalStatus'] = !hasMinimumPosts
    ? 'not-enough-posts'
    : engagedPosts === 0
      ? 'no-engagement'
...
const buildFollowerGrowthSeries = (posts: AnalyticsPostInsight[]): AnalyticsFollowerTrendPoint[] => {
  const points = posts
    .filter((post) => post.publishedTime && post.followersAtPostTime !== null)
    .sort(
      (left, right) =>
        new Date(left.publishedTime || 0).getTime() - new Date(right.publishedTime || 0).getTime()
    )
    .map((post) => ({
      date: post.publishedTime as string,
      label: toDateLabel(post.publishedTime as string),
      value: post.followersAtPostTime as number,
    }));
...
const getFollowerGrowthValue = (points: AnalyticsFollowerTrendPoint[]) => {
  if (points.length < 2) {
    return null;
  }
...
const buildPlatformComparison = (posts: AnalyticsPostInsight[]): AnalyticsPlatformComparison[] => {
  const grouped = new Map<string, AnalyticsPostInsight[]>();
  ...
  const impressionValues = [...grouped.values()].map((entries) =>
    entries.reduce((total, post) => total + post.impressions, 0) / Math.max(entries.length, 1)
  );
...
const buildContentTypeInsights = (posts: AnalyticsPostInsight[]) =>
  [...posts.reduce((map, post) => {
    ...
    return map;
  }, new Map<string, { postType: string; posts: number; engagementRateTotal: number; impressions: number; reach: number; saves: number }>()).values()]
    .map((entry) => ({
      ...entry,
...
const buildContentTopicInsights = (posts: AnalyticsPostInsight[]) =>
  [...posts.reduce((map, post) => {
    ...
    return map;
  }, new Map<string, { topic: string; posts: number; engagementRateTotal: number; comments: number; saves: number }>()).values()]
    .map((entry) => ({
      ...entry,
      averageRate: entry.posts ? entry.engagementRateTotal / entry.posts : 0,
    }))
...
const buildInsightCards = (
  ...
  previousPosts: AnalyticsPostInsight[],
  platformComparison: AnalyticsPlatformComparison[],
  bestTime: AnalyticsBestTimeInsight,
  trendSeries: AnalyticsTrendPoint[],
  range: DateRangeWindow,
  learningProfiles: AnalyticsLearningProfile[] = []
): AnalyticsInsightCard[] => {
  ...
  const followerGrowthChange = computeChangePercent(
  ...
  const weakestTrend = [...trendSeries]
    .map((point) => ({
      ...point,
      engagementRate:
        point.reach > 0 ? Number(((point.engagements / point.reach) * 100).toFixed(2)) : null,
    }))
    .filter((point) => point.engagementRate !== null)
    .sort((left, right) => (left.engagementRate as number) - (right.engagementRate as number))[0];
  ...
  for (const profile of learningProfiles.slice(0, 2)) {
    ...
    cards.push({
      ...
      confidence:
        topPattern && topPattern.sampleSize >= 4
          ? 'high'
          : topPattern
            ? 'medium'
            : 'low',
  ...
  if (topPost) {
    cards.push({
      ...
      confidence: posts.length >= ANALYTICS_LEARNING_MIN_POSTS ? 'high' : 'medium',
      tone: 'positive',
    });
  }
  ...
  if (lowestPost && lowestPost.id !== topPost?.id) {
    cards.push({
      ...
      confidence: posts.length >= ANALYTICS_LEARNING_MIN_POSTS ? 'medium' : 'low',
      tone: 'warning',
    });
  }
  ...
  if (bestTime.hasEnoughData) {
    ...
    const hasMinimumPosts =
      bestTime.postsConsidered >= bestTime.minimumPostsRequired;
    ...
    cards.push({
      ...
      description: bestTime.summary,
      supportingMetric:
        bestTime.signalStatus === 'no-engagement'
  ...
  if (bestPlatform) {
    const gap =
      secondPlatform && bestPlatform.score > 0
  ...
  if (bestContentType) {
    const liftVsSecondBest =
      secondContentType && bestContentType.averageRate > 0
        ? Number(
            (
              ((bestContentType.averageRate - secondContentType.averageRate) /
  ...
  cards.push({
    ...
    tone:
      currentFollowerGrowth !== null && currentFollowerGrowth > 0 && (followerGrowthChange ?? 0) >= 0
        ? 'positive'
        : 'neutral',
  });
...
const buildLearningDashboard = (
  learningProfiles: AnalyticsLearningProfile[],
  scopedPostsCount: number,
  platformScope: AnalyticsPlatformScope
): AnalyticsLearningDashboard => {
  const sortedProfiles = [...learningProfiles].sort(
    (left, right) =>
  ...
  const topProfile =
    ...
    null;
  const topPattern =
    topProfile?.patterns.find((pattern) => !isUnknownHookPattern(pattern)) ?? null;
  const weakPattern = topProfile?.weakPatterns[0] ?? null;
  const topRecommendationReason =
    topProfile && typeof topProfile.analyticsContext.recommendationReason === 'string'
      ? (topProfile.analyticsContext.recommendationReason as string)
      : null;
  const topRecommendationAccuracy =
    topProfile && typeof topProfile.analyticsContext.recommendationAccuracy === 'number'
      ? (topProfile.analyticsContext.recommendationAccuracy as number)
      : null;
  const topRecommendationAccuracyLabel =
    topProfile && typeof topProfile.analyticsContext.recommendationAccuracyLabel === 'string'
      ? (topProfile.analyticsContext.recommendationAccuracyLabel as string)
      : null;
  const storedRecommendation = topProfile?.recommendationText?.trim() ?? null;
  const genericRecommendationPattern =
    /keep testing .* consistent cadence so prixmoai can lock onto a stronger winner/i;
  const fallbackRecommendation = topPattern
    ? `Next, try more ${formatLearningPatternLabel(
        topPattern.dimension,
        topPattern.label
      )}. They are landing better than your usual results right now.`
    : weakPattern
  ...
  const topRecommendation =
    storedRecommendation && !genericRecommendationPattern.test(storedRecommendation)
      ? storedRecommendation
      : fallbackRecommendation;
  const isReady = scopedPostsCount >= ANALYTICS_LEARNING_MIN_POSTS;
  const missingDataMessage = isReady
    ? null
    : `Publish ${ANALYTICS_LEARNING_MIN_POSTS} scheduler posts to unlock smarter recommendations.`;
  ...
  const confidence =
    !isReady || !topProfile
      ...
      : scopedPostsCount >= 12
        ? 'high'
        : scopedPostsCount >= ANALYTICS_LEARNING_MIN_POSTS
          ? 'medium'
          : 'low';
  ...
  return {
    ...
    minimumPostsRequired: ANALYTICS_LEARNING_MIN_POSTS,
    missingDataMessage,
...
export const getAnalyticsDashboard = async (
  ...
  const [
    ...
    { data: socialAccountsData, error: socialAccountsError },
    rawAnalyticsRows,
    rawAudienceSnapshots,
    learningProfiles,
  ] =
    await Promise.all([
      client
      ...
      getAnalyticsAudienceSnapshotsByUserId(client, userId),
      listAnalyticsLearningProfilesByUser(client, userId, {
  ...
  const scheduledPosts = ((scheduledPostsData ?? []) as ScheduledPostAnalyticsRow[]).map((row) => ({
    ...row,
    published_at: resolveScheduledPostPublishedAt(row),
  }));
  const socialAccounts = (socialAccountsData ?? []) as SocialAccountAnalyticsRow[];
  ...
  const audienceSnapshots = rawAudienceSnapshots.filter((snapshot) =>
    platformScope === 'all' ||
  ...
  const connectedPlatforms = [...new Set(
    socialAccounts
      ...
      .filter((platform): platform is Exclude<AnalyticsPlatformScope, 'all'> => Boolean(platform))
  )];

  const analyticsRows = rawAnalyticsRows
    .map((row) => {
      const rawRow = row as AnalyticsData;
      ...
      const socialAccount = scheduledPost
        ...
        : undefined;
      const platform =
        normalizePlatformKey(rawRow.platform) ||
      ...
      return {
        ...
        engagements,
        engagementRateRatio,
      } satisfies EnrichedAnalyticsRow;
    })
    .filter((row) => platformScope === 'all' || row.platformKey === platformScope);

  const currentRows = analyticsRows.filter((row) =>
    isWithinRange(row.recordedAt, range.start, range.end)
  );
  const previousRows = analyticsRows.filter((row) =>
    isWithinRange(row.recordedAt, range.previousStart, range.previousEnd)
  );
  const currentAudienceSnapshots = audienceSnapshots.filter((snapshot) =>
    isWithinRange(snapshot.recordedAt, range.start, range.end)
  );
  const previousAudienceSnapshots = audienceSnapshots.filter((snapshot) =>
    isWithinRange(snapshot.recordedAt, range.previousStart, range.previousEnd)
  );
  const currentSnapshots = latestByKey(currentRows, (row) => row.postKey, (row) => row.recordedAt);
  const previousSnapshots = latestByKey(
    previousRows,
    (row) => row.postKey,
    (row) => row.recordedAt
  );
  ...
  const previousPublishedPosts = scheduledPosts.filter((post) => {
    ...
    return (
      post.status === 'published' &&
      isWithinRange(
        resolveScheduledPostPublishedAt(post),
        range.previousStart,
        range.previousEnd
      )
    );
  ...
  const platformBreakdownForMetric = (
    selector: (row: EnrichedAnalyticsRow) => number
  ): Partial<Record<Exclude<AnalyticsPlatformScope, 'all'>, number>> => {
    const values: Partial<Record<Exclude<AnalyticsPlatformScope, 'all'>, number>> = {};
  ...
  const sumMetric = (rows: EnrichedAnalyticsRow[], selector: (row: EnrichedAnalyticsRow) => number) =>
  ...
  const currentFollowerSeriesFromSnapshots = buildFollowerGrowthSeriesFromAudienceSnapshots(
    currentAudienceSnapshots
  );
  const previousFollowerSeriesFromSnapshots = buildFollowerGrowthSeriesFromAudienceSnapshots(
    previousAudienceSnapshots
  );
  const currentFollowerSeriesFallback = buildFollowerGrowthSeries(
    currentSnapshots.map((row) => ({
      ...row,
      performanceScore: 0,
      keywords: [],
      trend: [],
    } as AnalyticsPostInsight))
  );
  const previousFollowerSeriesFallback = buildFollowerGrowthSeries(
    previousSnapshots.map((row) => ({
      ...row,
      performanceScore: 0,
      keywords: [],
      trend: [],
  ...
  const currentFollowerSeries =
    currentFollowerSeriesFromSnapshots.length > 0
      ? currentFollowerSeriesFromSnapshots
      : currentFollowerSeriesFallback;
  const previousFollowerSeries =
    previousFollowerSeriesFromSnapshots.length > 0
  ...
  const overview = {
    ...
    engagementRate: buildMetric(
      currentReach > 0 ? Number(((currentEngagements / currentReach) * 100).toFixed(2)) : null,
      previousReach > 0
        ? Number(((previousEngagements / previousReach) * 100).toFixed(2))
        : null,
      buildSparkline(trendSeries, (point) =>
    ...
    postsPublished: buildMetric(
      currentPublishedPosts.length,
      previousPublishedPosts.length,
      buildSparkline(
        trendSeries,
        (point) =>
          currentPublishedPosts.filter((post) =>
            isWithinRange(
              resolveScheduledPostPublishedAt(post),
              point.date,
              addIstDays(new Date(point.date), 1).toISOString()
            )
          ).length
      )
    ),
  ...
  const postTrendMap = new Map<string, AnalyticsPostTrendPoint[]>();
  ...
  const postScores = buildAnalyticsPerformanceScores(
    currentSnapshots.map((row) => ({
      id: row.id,
      likes: row.likes,
      comments: row.comments,
  ...
  const posts: AnalyticsPostInsight[] = currentSnapshots
    .map((row) => {
    ...
    .sort((left, right) => {
      ...
      if (leftScore && rightScore) {
        return compareAnalyticsPerformanceScores(
          {
            ...leftScore,
            likes: left.likes,
  ...
  const bestTime = buildHeatmap(
    posts.filter((post) => Boolean(post.publishedTime))
  );
  const platformComparison = buildPlatformComparison(posts);
  const audienceFollowerSeries =
    currentFollowerSeries.length
      ? currentFollowerSeries
  ...
  const latestAudienceSnapshots = latestByKey(
    currentAudienceSnapshots,
    (snapshot) => snapshot.socialAccountId,
    (snapshot) => snapshot.recordedAt
  );
  const ageDistribution = mergeAudienceBreakdownItems(
    latestAudienceSnapshots.map((snapshot) => snapshot.ageDistribution)
  );
  const genderDistribution = mergeAudienceBreakdownItems(
    latestAudienceSnapshots.map((snapshot) => snapshot.genderDistribution)
  );
  const ageGenderBreakdown =
    ageDistribution.length || genderDistribution.length
      ? mergeAudienceBreakdownItems([
  ...
  const topLocations = mergeAudienceBreakdownItems(
    latestAudienceSnapshots.map((snapshot) => snapshot.topLocations)
  );
  const mergedTopLocations =
    topLocations.length ? topLocations : fallbackTopLocations;
  const followerGrowthValue = getFollowerGrowthValue(audienceFollowerSeries);
  const profileVisits = aggregateLatestAudienceMetric(
    currentAudienceSnapshots,
    (snapshot) => snapshot.profileVisits
  );
  const pageLikes = aggregateLatestAudienceMetric(
    currentAudienceSnapshots,
    (snapshot) => snapshot.pageLikes
  );
  const activeHoursHeatmap = buildActiveHoursHeatmapFromSnapshots(
    currentAudienceSnapshots,
    bestTime.heatmap
  );
  const audience = {
    hasAudienceData:
      audienceFollowerSeries.length > 1 ||
      bestTime.hasEnoughData ||
  ...
  const learning = buildLearningDashboard(
    learningProfiles,
    posts.length,
    platformScope
  );
