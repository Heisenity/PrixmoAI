...
const REFERENCE_SIMILARITY_PATTERN =
  /\b(similar|same|like\s+(?:this|the|that)\s+image|like\s+the\s+reference|as\s+per\s+the\s+(?:image|reference)|based\s+on\s+the\s+(?:image|reference)|match\s+the\s+(?:image|reference)|use\s+the\s+(?:image|reference)\s+below|shown\s+below|pasted\s+below)\b/i;

const IMAGE_MEMORY_PROMPT_TYPE_PRIORITY: Record<string, number> = {
  'image-prompt': 100,
...
const buildLinkedCampaignImageDirection = (
  contentContext: ProductInput | null | undefined
): string[] => {
  if (!contentContext) {
    return [];
  }
...
const buildConnectedAccountImageDirection = (
  brandMemories: BrandMemoryMatch[] | undefined
): string[] => {
  const connectedAccountMemory = brandMemories?.find(
  ...
  const connectedAccountSummary = clampDirectiveText(
    connectedAccountMemory.contentText,
    320
  );
  const lines = [
    ...
    asString(visualDna?.composition)
      ? `- Composition pattern: ${clampDirectiveText(
          asString(visualDna?.composition),
          180
        )}`
      : null,
    asString(visualDna?.background)
      ? `- Background pattern: ${clampDirectiveText(
          asString(visualDna?.background),
          180
        )}`
      : null,
    ...
    asString(visualDna?.textUsage)
      ? `- Text usage tendency: ${clampDirectiveText(
          asString(visualDna?.textUsage),
          180
        )}`
      : null,
...
const getImageMemoryDirectiveLabel = (memoryType: string) => {
  switch (memoryType) {
    case 'image-prompt':
...
const extractImageMemoryDirectiveText = (memory: BrandMemoryMatch) => {
  if (memory.memoryType === 'image-prompt') {
    const promptMatch = memory.contentText.match(
      /Prompt:\s*(.+?)(?:\.\s*Product description:|$)/i
    );
  ...
  if (memory.memoryType === 'platform-performance-insight') {
    ...
    const recommendationText = clampDirectiveText(
      asString(signals?.recommendationText),
      220
    );
...
const selectBrandMemoriesForImagePrompt = (
  brandMemories: BrandMemoryMatch[] | undefined
) => {
  ...
  return (brandMemories ?? [])
    ...
    .sort((left, right) => {
      const rightPriority =
        IMAGE_MEMORY_PROMPT_TYPE_PRIORITY[right.memoryType] ?? 0;
      const leftPriority =
        IMAGE_MEMORY_PROMPT_TYPE_PRIORITY[left.memoryType] ?? 0;
...
const buildRetrievedImageMemoryDirection = (
  brandMemories: BrandMemoryMatch[] | undefined
): string[] => {
...
const buildPlatformImageDirection = (platform: string | null | undefined) => {
...
const BACKGROUND_STYLE_DIRECTIONS: Record<string, string> = {
  'clean studio background':
    'seamless clean studio backdrop, softbox lighting, controlled shadows, uncluttered negative space, premium commercial product photography',
  'soft luxury shadow set':
    'soft luxury shadow set with layered diffused shadows, warm high-end lighting, subtle dimensional depth, elegant premium surface treatment',
  'editorial marble surface':
    'editorial marble surface with natural veining, reflective highlights, magazine-quality styling, polished luxury still-life composition',
  'muted gradient backdrop':
    'muted gradient backdrop with smooth tonal falloff, soft atmospheric depth, refined modern color transitions, no harsh banding',
  'minimal industrial texture':
    'minimal industrial texture with refined concrete or brushed metal, clean geometry, restrained grit, premium utilitarian finish',
  'premium showroom interior':
    'premium showroom interior with curated display lighting, expensive materials, clean spatial depth, high-end retail presentation',
  'warm lifestyle room':
    'warm lifestyle room with believable natural light, tasteful decor, soft lived-in atmosphere, product placed naturally in context',
  'luxury boutique display':
    'luxury boutique display with elevated shelving, soft accent lighting, refined retail styling, elegant premium merchandising',
  'urban street backdrop':
    'urban street backdrop with controlled cinematic depth, believable street texture, tasteful blur, no distracting signage or text',
  'nature daylight setting':
    'nature daylight setting with soft natural light, organic textures, gentle depth of field, fresh outdoor premium mood',
  'festive indian decor':
    'festive Indian decor with tasteful warm lights, premium traditional accents, rich but uncluttered styling, celebratory atmosphere',
  'modern cafe table scene':
    'modern cafe table scene with warm ambient light, clean tabletop composition, lifestyle realism, tasteful depth in the background',
  'dark cinematic spotlight':
    'dark cinematic spotlight with controlled rim light, moody graphite shadows, dramatic subject separation, premium filmic contrast',
  'pastel paper sweep':
    'pastel paper sweep with smooth curved backdrop, soft studio lighting, gentle shadows, clean commercial product presentation',
  'wooden tabletop studio':
    'wooden tabletop studio with refined grain texture, warm directional light, realistic contact shadows, premium handcrafted feel',
  'concrete wall and floor':
    'concrete wall and floor with modern matte texture, clean architectural lines, soft shadow gradient, premium minimalist realism',
  'high-end retail shelf':
    'high-end retail shelf with neat product staging, soft shelf lighting, premium store ambience, clean background hierarchy',
  'minimal tech gradient':
    'minimal tech gradient with sleek blue-gray lighting, subtle futuristic depth, clean reflections, premium AI SaaS visual language',
  'vintage film backdrop':
    'vintage film backdrop with cinematic texture, tasteful retro color grading, subtle grain, old-studio atmosphere without visible text',
  'custom background':
    'custom background directed by the user, interpreted literally first, then polished into a coherent premium scene',
};

const normalizePromptText = (value: string | null | undefined) =>
  typeof value === 'string' ? value.replace(/\s+/g, ' ').trim() : '';

const buildAdvancedBackgroundDirection = (input: ResolvedGenerateImageInput): string => {
  ...
  const presetDirection =
    BACKGROUND_STYLE_DIRECTIONS[selectedStyle.toLowerCase()] ?? selectedStyle;
...
const buildReferenceImageDirection = (input: ResolvedGenerateImageInput): string[] => {
  ...
  const combinedReferenceBrief = [
    ...
    .filter(Boolean)
    .join(' ');
  const userWantsCloseReferenceMatch =
    REFERENCE_SIMILARITY_PATTERN.test(combinedReferenceBrief);
